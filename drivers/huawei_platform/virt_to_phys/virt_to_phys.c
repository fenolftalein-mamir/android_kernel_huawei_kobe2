	...
	unsigned long addr)
{
	...
	size += snprintf_s(buf + size, PAGE_SIZE - size, PAGE_SIZE - size - 1,
		"pgd = %p\n", mm->pgd);
	...
	do {
		...
		if (pgd_bad(*pgd)) {
			size += snprintf_s(buf + size, PAGE_SIZE - size,
				PAGE_SIZE - size - 1, "(bad)");
			break;
		}
	...
	char *buf)
{
	...
	size = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%u, 0x%lx, %u\n",
	...
	const char *buf, size_t count)
{
	...
	while ((len > 0) && (args < ARGS_3)) {
		...
		switch (args) {
		case ARGS_0:
			if (sscanf_s(head, "%u", &g_pid) != 1)
				return -EINVAL;
			break;
		case ARGS_1:
			if (sscanf_s(head, "%lx", &g_virt_addr) != 1)
				return -EINVAL;
			break;
		case ARGS_2:
			if (sscanf_s(head, "%u", &g_virt_len) != 1)
				return -EINVAL;
			break;
		default:
