...
tfa98xx_set_mute_tfa2(struct tfa_device *tfa, enum Tfa98xx_Mute mute)
{
	...
	if (tfa->dev_ops.set_mute == NULL) {
        pr_err("%s set_mute is null ", __func__);
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	manstate = TFA_GET_BF(tfa, MANSTATE);
	while((manstate >= ENABLE_PLL_STATE) &&
			(retry < MANSTATE_RETRY_TIMES)) {
		msleep_interruptible(10);
		manstate = TFA_GET_BF(tfa, MANSTATE);
		retry++;
	}
	if (retry >= MANSTATE_RETRY_TIMES)
		pr_info("%s state machine to powerdown.MANSTATE %d over %d\n",
